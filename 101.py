...
class JupyterBackup:
                ...
                try:
                    ...
                    if not last or last[0] != md5_hash:
                        con.execute(
                            ...
                            [f_path.name, created_at, content, md5_hash, message],
                        )
                        con.commit()
    ...
    def restore(self, target_filename, target_timestamp=None):
        ...
        with sqlite3.connect(self.db_filename) as con:
            ...
            if result:
                ...
                if target_timestamp:
                    print(
                        f"⚠️ No backup found for '{target_filename}' matching timestamp '{target_timestamp}'."
                    )
                else:
                    print(
                        f"⚠️ No backups found for '{target_filename}'. Check the spelling."
                    )
    ...
    def list_backups(self, specific_file=None):
        ...
        with sqlite3.connect(self.db_filename) as con:
            if specific_file:
                results = con.execute(
                    "SELECT inserted, message FROM backup1 WHERE filename = ? ORDER BY inserted DESC",
                    [specific_file],
                ).fetchall()
                ...
                results = con.execute("""
                        ...
                        SELECT filename, inserted, message, 
                        COUNT(*) OVER(PARTITION BY filename) as versions, 
                        ROW_NUMBER() OVER(PARTITION BY filename ORDER BY inserted DESC) as rn 
                        FROM backup1 
                        ) 
                        SELECT filename, versions, inserted as last_backup, message as Message 
                        FROM RankedBackups 
                        WHERE rn = 1 
                ...
                print(f"\n{'Filename':<30} | {'Versions':<10} | {'Last Backup':<19} | {'Message'}")
                print("-" * 80)
                for row in results:
                    fname = row[0] if len(row[0]) <= 29 else row[0][:26] + "..."
                    print(f"{fname:<30} | {row[1]:<10} | {row[2]} | {row[3]}")
    ...
    def _render_ipynb(self, raw_json_str):
        ...
        try:
            ...
            print(
                "❌ Error parsing notebook file: Stored file contents are not valid JSON."
            )
...
def main():
    ...
    backup_parser = subparsers.add_parser(
        "backup", help="Backup all tracked files in the current directory"
    )
    backup_parser.add_argument("-m", help="write a short Message")

    restore_parser = subparsers.add_parser(
        "restore", help="Restore the latest or a specific version of a file safely"
    )
    restore_parser.add_argument("filename", help="Exact name of the file to restore")
    restore_parser.add_argument(
        "-t", "--timestamp", help="Timestamp to restore", default=None
    )

    list_parser = subparsers.add_parser(
        "list", help="List all backed up files and their metadata"
    )
    list_parser.add_argument(
        "-f",
        "--filename",
        help="List all available timestamps for a specific file",
        default=None,
    )

    view_parser = subparsers.add_parser(
        "view", help="Print the contents of a backed up file to the terminal"
    )
    view_parser.add_argument("filename", help="Exact name of the file to view")
    view_parser.add_argument(
        "-t", "--timestamp", help="Timestamp to view", default=None
    )
